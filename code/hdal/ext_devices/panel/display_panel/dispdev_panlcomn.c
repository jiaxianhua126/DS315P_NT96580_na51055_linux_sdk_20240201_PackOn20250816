...
#define IOC_LCD_DIR_NORMAL_OSDLAYER                	_IOWR('w', 1, unsigned int)
#define IOC_LCD_DIR_MIRROR_OSDLAYER                 _IOWR('w', 2, unsigned int)
#define IOC_LCD_DIR_FLIP_OSDLAYER                   _IOWR('w', 3, unsigned int)
#define IOC_LCD_DIR_MIRROR_FLIP_OSDLAYER            _IOWR('w', 4, unsigned int)
#define IOC_LCD_DIR_NORMAL_VIDEOLAYER              	_IOWR('w', 5, unsigned int)
#define IOC_LCD_DIR_MIRROR_VIDEOLAYER               _IOWR('w', 6, unsigned int)
#define IOC_LCD_DIR_FLIP_VIDEOLAYER                 _IOWR('w', 7, unsigned int)
#define IOC_LCD_DIR_MIRROR_FLIP_VIDEOLAYER          _IOWR('w', 8, unsigned int)
...
ER dispdev_set_display(DISPDEV_IOCTRL p_disp_dev_control, T_LCD_INF lcd_inf, T_LCD_PARAM *p_mode, PINMUX_FUNC_ID pin_func_id)
{
	...
	if (p_panel->b_ycbcr_format) {
		if (((p_ide->pinmux_select_lcd & PINMUX_LCD_SEL_CCIR601_16BITS) == PINMUX_LCD_SEL_CCIR601_16BITS) || ((p_ide->pinmux_select_lcd & PINMUX_LCD_SEL_CCIR656_16BITS) == PINMUX_LCD_SEL_CCIR656_16BITS)) {
		...
		if ((p_ide->pinmux_select_lcd & PINMUX_LCD_SEL_RGB_16BITS) == PINMUX_LCD_SEL_RGB_16BITS) {
...
#if defined __KERNEL__
static long lcdmirrorflip_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	PDISP_OBJ p_disp_obj;
	p_disp_obj = disp_get_display_object(DISP_1);

	switch (cmd) {
	case IOC_LCD_DIR_NORMAL_OSDLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
            printk("lcdmirrorflip_ioctl: IOC_LCD_DIR_NORMAL_OSDLAYER\r\n");
			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_NORMAL;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_OSD1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	     break;

	case IOC_LCD_DIR_MIRROR_OSDLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};

			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_HRZ_FLIP;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_OSD1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	     break;

	case IOC_LCD_DIR_FLIP_OSDLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};

			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_VTC_FLIP;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_OSD1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	     break;

	case IOC_LCD_DIR_MIRROR_FLIP_OSDLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
            printk("lcdmirrorflip_ioctl: IOC_LCD_DIR_MIRROR_FLIP_OSDLAYER\r\n");
			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_ROT_180;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_OSD1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	     break;

	case IOC_LCD_DIR_NORMAL_VIDEOLAYER:
	 	if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
            printk("lcdmirrorflip_ioctl: IOC_LCD_DIR_NORMAL_VIDEOLAYER\r\n");
			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_NORMAL;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_VDO1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	     break;

	case IOC_LCD_DIR_MIRROR_VIDEOLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
	...
	case IOC_LCD_DIR_FLIP_VIDEOLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
	...
	case IOC_LCD_DIR_MIRROR_FLIP_VIDEOLAYER:
		if(p_disp_obj) {
			DISPLAYER_PARAM LyrParam = {0};
            printk("lcdmirrorflip_ioctl: IOC_LCD_DIR_MIRROR_FLIP_VIDEOLAYER\r\n");
			LyrParam.SEL.SET_OUTDIR.buf_out_dir = DISPOUTDIR_ROT_180;
			p_disp_obj->disp_lyr_ctrl(DISPLAYER_VDO1, DISPLAYER_OP_SET_OUTDIR, &LyrParam); //it will call disp_translateBufAddress
			p_disp_obj->load(TRUE);
		 }
	...
	default:
	     printk("lcdmirrorflip_ioctl: wrong operate\r\n");
	     break;
	}
...
static const struct file_operations lcdmirrorflip_fops = {
	.owner          = THIS_MODULE,
	.unlocked_ioctl	= lcdmirrorflip_ioctl,
};

struct miscdevice lcdmirrorflip_miscdev = {
	.minor          = MISC_DYNAMIC_MINOR,
	.name           = "lcdmirrorflip",
	.fops           = &lcdmirrorflip_fops,
};
#endif

#if defined __KERNEL__
EXPORT_SYMBOL(dispdev_set_display);
EXPORT_SYMBOL(lcdmirrorflip_miscdev);
#endif
#if 0
/*
    Display close

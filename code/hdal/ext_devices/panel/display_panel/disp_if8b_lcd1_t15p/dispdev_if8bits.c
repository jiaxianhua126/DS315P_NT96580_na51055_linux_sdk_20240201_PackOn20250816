...
static ER 		dispdev_user_device_ctrl(UINT32 user_data);
...
static DISPDEV_OBJ     disp_dev_if8bits_obj = { dispdev_open_if8bits, dispdev_close_if8bits, dispdev_set_device_rotate, dispdev_write_lcd_reg, dispdev_read_lcd_reg, dispdev_get_lcd_size, dispdev_set_if8bits_ioctrl, { NULL, NULL, dispdev_user_device_ctrl } };
volatile DISPDEV_IOCTRL  p_if8bits_io_control1;
#elif (DISPLCDSEL_IF8BITS_TYPE == DISPLCDSEL_IF8BITS_LCD2)
static DISPDEV_OBJ     disp_dev_if8bits_obj2 = { dispdev_open_if8bits, dispdev_close_if8bits, dispdev_set_device_rotate, dispdev_write_lcd_reg, dispdev_read_lcd_reg, dispdev_get_lcd_size, dispdev_set_if8bits_ioctrl, { NULL, NULL, dispdev_user_device_ctrl } };
...
static void disp_device_init(T_LCD_PARAM *p_mode)
{
	...
	if (dev_io_ctrl.SEL.GET_REG_IF.lcd_ctrl == DISPDEV_LCDCTRL_SIF) {
		...
		if (dev_sif_param != NULL) {
			//printk("call dispdev_get_lcd_sif_param\r\n");
	...
	if (dev_io_ctrl.SEL.GET_PANEL_ADJUST.pfp_adjust != NULL) {
		...
		for (i = 0; i < p_mode->n_cmd; i++) {
			if (p_mode->p_cmd_queue[i].ui_address == CMDDELAY_MS) {
				...
				//printk("p_mode->p_cmd_queue[i].ui_address = %X,p_mode->p_cmd_queue[i].ui_value = %X",p_mode->p_cmd_queue[i].ui_address,p_mode->p_cmd_queue[i].ui_value);
...
static ER dispdev_open_if8bits(void)
{
	...
	logo_en = (((int) dev_io_ctrl.SEL.GET_LOGOMODE.b_en_logo) & first_init);
	...
	if (lcd_mode == PINMUX_LCDMODE_CCIR656) {
		...
		*/
		...
		if (p_mode->ide.interlace == TRUE) {
			...
			ui_odd_start = p_mode->panel.ui_vsync_active_period + p_mode->panel.ui_vsync_back_porch_odd + p_mode->panel.ui_vsync_total_period;
		}
		...
		if (p_mode->ide.interlace == TRUE) {
			...
			if (p_mode->panel.ui_hsync_back_porch == 0) { //special case
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_odd_start    = ui_odd_start - 1;  //ide odd,even = 0x24, 0x25
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_odd_end      = (p_mode->panel.ui_vsync_back_porch_odd) - 1; //=p_mode.panel.ui_vsync_back_porch_odd / 2
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_even_start   = (p_mode->panel.ui_vsync_active_period) + (p_mode->panel.ui_vsync_back_porch_even) - 1;   //=p_mode.panel.ui_vsync_active_period / 2 + p_mode.panel.ui_vsync_back_porch_odd / 2
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_even_end     = p_mode->panel.ui_vsync_total_period + p_mode->panel.ui_vsync_back_porch_even - 1;
				...
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_even_start   = (p_mode->panel.ui_vsync_active_period) + (p_mode->panel.ui_vsync_back_porch_even);   //=p_mode.panel.ui_vsync_active_period / 2 + p_mode.panel.ui_vsync_back_porch_odd / 2
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_even_end     = p_mode->panel.ui_vsync_total_period + p_mode->panel.ui_vsync_back_porch_even;
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_start  = 0;//(p_mode->panel.ui_vsync_sync_width);
				dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_end    = 0;//ui_field_end;				
			}
		}
	...
	if (logo_en == 0) {
		//printk("-------------------call dispdev_set_display--------------------\r\n");
...
#if defined(__KERNEL__)
    if ((!fastboot) && (logo_en == 0)) 
#endif
	{
		//printk("-------------------call disp_device_init------------------------\r\n");
...
static void dispdev_write_lcd_reg(UINT32 ui_addr, UINT32 ui_value)
{
	...
	if (dev_io_ctrl.SEL.GET_REG_IF.lcd_ctrl == DISPDEV_LCDCTRL_GPIO) {
		//printk("call DISPDEV_LCDCTRL_GPIO\r\n");
		...
		//printk("call DISPDEV_LCDCTRL_SIF\r\n");

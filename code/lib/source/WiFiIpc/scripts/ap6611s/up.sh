...
if [ "$1" == "ap" ]; then
	...
	echo -n /lib/firmware/fw_syn43711a0_sdio.bin > /sys/module/bcmdhd/parameters/firmware_path
	echo -n /lib/firmware/nvram_ap6611s.txt > /sys/module/bcmdhd/parameters/nvram_path

	ifconfig wlan0 up
	#echo interface=wlan0 > /var/run/hostapd.conf
	#echo driver=nl80211 >> /var/run/hostapd.conf
	#echo ssid=AndroidAP >> /var/run/hostapd.conf
	...
	#hostapd -ddd /var/run/hostapd.conf &

...
#if 0
static INT32 UINet_HFSheaderCb(UINT32 headerAddr, UINT32 headerSize, CHAR *filepath, CHAR *mimeType, void *reserved)
{
	char *headerstr;
	...
	if (!strncmp(headerstr, "GET", strlen("GET"))) {
		...
		} else if (!strncmp(headerstr, "GET /cgi-bin/hisnet/client.cgi?-operation=register&-ip=192.168.0.20", strlen("GET /cgi-bin/hisnet/client.cgi?-operation=register&-ip=192.168.0.20"))) {
			DBG_DUMP("http get custom\r\n");
			return CYG_HFS_CB_HEADER_RETURN_CUSTOM;
		} else {
			return CYG_HFS_CB_HEADER_RETURN_DEFAULT;
		...
		DBG_DUMP("http post\r\n");
		if (!strncmp(headerstr, "POST /cgi-bin/hisnet/downloadedogdata.cgi", strlen("POST /cgi-bin/hisnet/downloadedogdata.cgi"))) {
			return CYG_HFS_CB_HEADER_RETURN_CUSTOM;
		}else if (!strncmp(headerstr, "POST / ", strlen("POST / "))) {
			// set the data mimetype
			strncpy(filepath, "/sdcard/upload.jpg", CYG_HFS_FILE_PATH_MAXLEN);
...
#endif
		//hfsObj.headerCb = UINet_HFSheaderCb;
...
BOOL UINet_AppIpc_Init(void)
{
	...
	WiFiIpcOpen.sharedMemAddr = 0;//OS_GetMempoolAddr(POOL_ID_WIFIIPC);
	WiFiIpcOpen.sharedMemSize = 0;//POOL_SIZE_WIFIIPC;
...
void UINet_StationStatus_CB(char *pIntfName, char *pMacAddr, int status)
{
	...
	if (status == NVT_WIFI_STA_STATUS_ASSOCIATED) {
		...
		DBG_DUMP("%s: A client(%02X:%02X:%02X:%02X:%02X:%02X) is disassociated\r\n",
				pIntfName, *pMacAddr, *(pMacAddr + 1), *(pMacAddr + 2), *(pMacAddr + 3), *(pMacAddr + 4), *(pMacAddr + 5));
#if(DHCP_LEASE_CNT==1)
		...
		DBG_DUMP("%s: A client(%02X:%02X:%02X:%02X:%02X:%02X) is port authorized\r\n",
				pIntfName, *pMacAddr, *(pMacAddr + 1), *(pMacAddr + 2), *(pMacAddr + 3), *(pMacAddr + 4), *(pMacAddr + 5));
		...
		DBG_DUMP("^RAP ready, total period = %d us\r\n", clock() - g_Wifi_time);
	}
//#NT#2016/03/23#Isiah Chang -begin
//#NT#add new Wi-Fi UI flow.
...
void UINet_Link2APStatus_CB(char *pIntfName, int status)
{
	if (status == NVT_WIFI_LINK_STATUS_CONNECTED) {
		...
		DBG_DUMP("STA ready, total period = %d us\r\n", clock() - g_Wifi_time);
	} else {
...
void UINet_Print_WSCD_Status(int status)
{
	switch (status) {
	case NOT_USED:
		DBG_IND("NOT_USED\r\n");
...
void UINet_WSCEvent_CB(int evt, int status)
{
	...
	if (evt == NVT_WPS_REINIT_EVENT) {
		...
		UINet_Print_WSCD_Status(status);
	} else {
...
{
#if _TODO//defined(_CPU2_ECOS_) && defined(_NETWORK_ON_CPU2_)
...
//#NT#2016/08/25#YongChang_Qui -end
    ...
    //#NT#RTSP_PUSH_MODE
    #if 0
    if (result == E_OK && mode == NET_AP_MODE) {
        //station mode should get IP can Rtsp init
		//result = UINet_RtspInit();
	}
	#else
	UINet_RtspInit();
...
#if 1 //defined(_CPU2_ECOS_) && defined(_NETWORK_ON_CPU2_)
	DBG_DUMP("set Lan %d\r\n", mode);

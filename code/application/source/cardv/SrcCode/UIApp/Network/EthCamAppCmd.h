...
#define FLG_ETHCAM_CMD_DISP_PIP					FLGPTN_BIT(8)// 256
#define FLG_ETHCAM_CMD_SENDFRM					FLGPTN_BIT(9)// 512
#define FLG_ETHCAM_CMD_INIT_SOCKET			FLGPTN_BIT(10)// 1024
...
typedef struct {
	UINT32  ACodec;  //AUDDEC_DECODER_AAC
	UINT32 	AudRate;
	UINT32 	AudCh;
	UINT32 	AudChCnt;
} ETHCAM_TX_AUDINFO;
...
typedef struct {
	UINT32 	Enable;		//EIS Enable
	UINT32 	Version;		//EIS Version
	UINT32 	SrcType;	//Rx 52x or 53x
	UINT32 	GyroSend;    //send gyro data
	UINT32 	LutSend;	//send lut data
	UINT32   LutSize;  	//VENDOR_EIS_LUT_SIZE_SEL
	UINT32	reserved[2];
} ETHCAM_CMD_EISINFO;
...
extern ETHCAM_TX_AUDINFO sEthCamTxAudInfo[ETHCAM_PATH_ID_MAX];
#if(defined(_NVT_ETHREARCAM_TX_))
extern ETHCAM_TX_SYS_INFO sEthCamSysInfo;
extern ETHCAM_TX_CODEC_SRCTYPE sEthCamCodecSrctype;
extern UINT32 g_EthCamTxIsAudCapEn;
extern UINT32 g_EthCamTxIsCloneWriteFileEn;
extern UINT32 g_uiEthCam_TxFwUpdate_FwAddr;
#endif
...
extern ETHCAM_CMD_EISINFO sEthCamTxEISInfo[ETHCAM_PATH_ID_MAX];
#endif
...
extern ID _SECTION(".kercfg_data")  ETHCAM_CMD_SNDFRAME_CLONE_SEM_ID;
#endif
#if (defined(_NVT_ETHREARCAM_RX_))
...
extern UINT32 EthCamCmdTsk_IsOpen(void);
...
extern INT32 EthCamCmdXML_parsing_default_format(char *xml_buf, void * output);
...
extern UINT32 EthCam_TxFwUpdate_RelTempBuffer(UINT32 addr);

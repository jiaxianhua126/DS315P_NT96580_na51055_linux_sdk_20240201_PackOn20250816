...
static THREAD_RETTYPE MovieAlgFunc_ADAS_Tsk(void)
{
	...
	//add by keytech
	hd_ret = ALGO_MANAGER_GetHandleByName(&adas_handle,"AlgoAdas");
...
void MovieAlgFunc_ADAS_Init(void)
{
	...
	memcpy(creatPara.algoName,name,strlen(name));
	...
	memcpy(rcw_creatPara.algoName,rcw_name,strlen(rcw_name));
...
ER MovieAlgFunc_ADAS_UninstallID(void)
{
	...
	delay_cnt = 50;//10
	movie_alg_adas_tsk_run = FALSE;
	while (is_movie_alg_adas_tsk_running && delay_cnt) {

...
#define DRVLCD_BRT_LEVEL_MAX      12
#define DRVLCD_BRT_LEVEL_INIT     8
...
static void GPIOMap_LCDReset(void)
{
	...
	gpio_setDir(GPIO_LCD_RESET, GPIO_DIR_OUTPUT); //dsi lcd for old machine
	gpio_setDir(S_GPIO_5, GPIO_DIR_OUTPUT); // dsi lcd for new machine
    gpio_setDir(P_GPIO_16, GPIO_DIR_OUTPUT); //digital lcd

	gpio_clearPin(GPIO_LCD_RESET);
    gpio_clearPin(S_GPIO_5);
    gpio_clearPin(P_GPIO_16);
	SwTimer_DelayMs(20);
	gpio_setPin(GPIO_LCD_RESET);
    gpio_setPin(S_GPIO_5);
    gpio_setPin(P_GPIO_16);
	SwTimer_DelayMs(10);


}

#define LCD_PROC "/dev/lcdpwm"
#define FD_INITIALIZATION_VAL (-1)
//static INT32 LCDpwmfd = FD_INITIALIZATION_VAL;
#define IOC_LCD_BACKLIGHT_ON              	_IOWR('w', 1, unsigned int) //duty cycle 100
#define IOC_LCD_BACKLIGHT_OFF              	_IOWR('w', 2, unsigned int)
INT32 User_LcdPwm_Init(void)
{
#if 0	
    INT32 s32Ret;

    if (LCDpwmfd != FD_INITIALIZATION_VAL) {
        DBG_ERR("Screen has been inited\n");
        return E_PAR;
    }
    
    DBG_DUMP("modprobe LCD backlight ctrl lcdpwm\n");
    s32Ret = nvt_system("modprobe lcd_ota7290b");
    if (E_OK != s32Ret) {
        DBG_ERR("modprobe lcdpwm: failed\n");
        return E_PAR;
    }
    LCDpwmfd = open(LCD_PROC, O_RDWR);
    if (LCDpwmfd < 0) {
...
INT32 User_LcdPwm_Uninit(void)
{
#if 0	
    INT32 ret;

    if (LCDpwmfd == FD_INITIALIZATION_VAL) {
        DBG_ERR("lcdpwm not initialized,no need to close\n");
        return E_PAR;
    }
    ret = close(LCDpwmfd);
    if (0 > ret) {
        DBG_ERR("LCDpwmfd[%d] close,fail\n",LCDpwmfd);
        return E_SYS;
    }
    LCDpwmfd = FD_INITIALIZATION_VAL;
    ret = nvt_system("rmmod lcd_ota7290b");
    if (E_OK != ret) {
        DBG_ERR("modprobe rmmod: failed\n");
        return E_PAR;
    }
#endif
...
INT32 User_LcdPwm_SetLuma(INT32 luma)
{
#if 0
    INT32 ret;
    INT32 level;
    DBGD(luma);
    if (luma < 0 || luma > 100) {
        DBG_ERR("HAL_SCREEN_SetLuma failed wrong param\n");
        return E_PAR;
    }
    level = luma;
    if (luma == 0) {
        ret = ioctl(LCDpwmfd, IOC_LCD_BACKLIGHT_OFF, NULL);
    } else {
        ret = ioctl(LCDpwmfd, IOC_LCD_BACKLIGHT_ON, &level);
    }
    if (-1 == ret) {
        DBG_ERR("HAL_SCREEN_SetBackLightState: failed luma:%d\n",level);
        return E_PAR;
    }
#endif
...
static void GPIOMap_TurnOnLCDBacklight(void)
{
	...
	if (g_LCDBacklightEn == FALSE) {
		...
		User_LcdPwm_SetLuma((g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl]));
        #endif
...
static void GPIOMap_SetLCDBacklightBrightLevel(INT32 uiLevel)
{
    ...
    if (g_LCDBacklightLvl > DRVLCD_BRT_LEVEL_MAX) {
		DBG_WRN("DrvLCDSetCtrl Warning! brightlvl=0x%02X out of range\r\n", g_LCDBacklightLvl);
        g_LCDBacklightLvl = DRVLCD_BRT_LEVEL_MAX;
	}
...
static void GPIOMap_DumpBacklight(void)
{
#if 0//(LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)

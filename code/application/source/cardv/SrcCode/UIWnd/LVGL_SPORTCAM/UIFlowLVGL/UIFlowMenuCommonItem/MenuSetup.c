...
static uint32_t msgbox_auto_close_time = 3000;

int MenuCustom_DeleteAll(UINT32 uiMessage, UINT32 uiParam)
{
    DBG_FUNC_BEGIN("uiParam=%d\r\n",uiParam);
	if (SetupExe_IsNoFile() == TRUE) {
		UIFlowWrnMsgAPI_Open_StringID(FLOWWRNMSG_ISSUE_NO_IMAGE, msgbox_auto_close_time);
...
// Align the LVGL setup list with the currently enabled SPORTCAM setup entries.

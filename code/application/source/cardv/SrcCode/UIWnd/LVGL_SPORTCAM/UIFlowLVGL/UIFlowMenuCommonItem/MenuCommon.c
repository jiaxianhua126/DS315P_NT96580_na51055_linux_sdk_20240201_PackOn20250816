...
TMDEF_BEGIN_OPTIONS(COMMON_LIVESTREAM)
TMDEF_OPTION_TEXT_S(COMMON_LIVESTREAM_OFF, TM_OPTION_ENABLE)
TMDEF_OPTION_TEXT_S(COMMON_LIVESTREAM_ON, TM_OPTION_ENABLE)
TMDEF_END_OPTIONS()
#endif
...
#endif
#if (LIVESTREAM_FUNC == ENABLE)
TMDEF_ITEM_TEXTID(COMMON_LIVESTREAM)
#endif
...
#if (LIVESTREAM_FUNC == ENABLE)
		case IDM_COMMON_LIVESTREAM:
			if ((uwOption == LIVESTREAM_ON) && (System_GetState(SYS_STATE_CURRMODE) == PRIMARY_MODE_MOVIE)) {
				SysSetFlag(FL_COMMON_LIVESTREAM, LIVESTREAM_ON);
				Ux_SendEvent(&UISetupObjCtrl, NVTEVT_EXE_WIFI_REQ_QR_DECODE, 1);
			}
			break;

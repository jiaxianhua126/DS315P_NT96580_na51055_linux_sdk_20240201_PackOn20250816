...
//#NT#Add UVC multimedia function.
	...
	if (System_GetState(SYS_STATE_CURRSUBMODE) == SYS_SUBMODE_UVC) {
		//if (ImageUnit_GetParam(&ISF_UsbUVAC, ISF_CTRL, UVAC_PARAM_CDC_ENABLE)) {
			USBCMD_MEMRANGE WorkBuf;
			WorkBuf.addr_pa = mempool_usbcmd_pa;
			WorkBuf.addr_va = mempool_usbcmd_va;
...
INT32 UIMenuWndWiFiMobileLinkOK_OnBackgroundDone(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	case NVTEVT_BKW_ETHCAM_SOCKET_OPEN:
	case NVTEVT_BKW_ETHCAM_SOCKETCLI_CMD_OPEN:
	case NVTEVT_BKW_ETHCAM_SOCKETCLI_DATA_OPEN:
	case NVTEVT_BKW_GET_ETHCAM_TX_INFO:
	case NVTEVT_BKW_TRIGGER_THUMB:
	case NVTEVT_BKW_TX_STREAM_START:

	case NVTEVT_BKW_GET_ETHCAM_TX_DECINFO:
	case NVTEVT_BKW_ETHCAM_SOCKETCLI_DISP_DATA_OPEN_START:
	case NVTEVT_BKW_ETHCAM_SOCKETCLI_DISP_DATA_OPEN:
	case NVTEVT_BKW_GET_ETHCAM_TX_RECINFO:
	case NVTEVT_BKW_ETHCAM_SOCKETCLI_REC_DATA_OPEN_START:

	case NVTEVT_BKW_ETHCAM_SYNC_TIME:

	case NVTEVT_BKW_ETHCAM_DEC_ERR:
	case NVTEVT_BKW_ETHCAM_RAW_ENCODE_RESULT:
	case NVTEVT_BKW_ETHCAM_SET_TX_SYSINFO:
		break;
...
INT32 UIMenuWndWiFiMobileLinkOK_OnUpdateInfo(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	extern INT32 UIFlowWndPhoto_OnUpdateInfo(VControl *, UINT32, UINT32 *);
	return UIFlowWndPhoto_OnUpdateInfo(pCtrl, paramNum, paramArray);
#else
	return NVTEVT_CONSUME;
#endif
}
